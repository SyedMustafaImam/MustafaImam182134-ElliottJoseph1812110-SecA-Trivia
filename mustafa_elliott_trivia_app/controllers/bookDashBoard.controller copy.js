...
exports.product_formcreate=function(req,res){
    res.render('createform',{page:'New Product', menuId:'createform', session:req.session})
};

//this function loads the view and finds all the documents in Product collections
exports.product_list=function(req,res){
    Product.find(function(err, product){
        res.render('list',{page:'Show All Products', menuId:'list', product:product, session:req.session});
    });
};
...
exports.product_updateform=function(req,res){
    Product.findById(req.params.id,function(err, product){
        if (err) return next(err);
        res.render('updateform',{page:'Update Product', menuId:'updateform',product:product,session:req.session});
    })
};
